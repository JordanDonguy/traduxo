...
function TranslationSection({
  ...
  const expression = translatedText.find(item => item.type === "expression")?.value ?? "";
  const mainTranslation = translatedText.find(item => item.type === "main_translation")?.value ?? "";
  const alternatives = translatedText.filter(item => item.type === "alternative").map(a => a.value);

  return (
    <div className="w-full max-w-[96%] sm:max-w-xl lg:max-w-3xl flex flex-col mt-12">
      {/* Input language + original text */}
      <article
        className={`flex gap-4 pr-4 bg-[var(--bg-2)] mb-8 rounded-md duration-500 ease-in-out transform font-semibold ${
      ...
      >
        ...
        <p className="text-xl min-h-8 flex items-center">{capitalizeFirstLetter(expression)}</p>
      </article>
      ...
      <article
        className={`flex flex-col gap-2 bg-[var(--bg-2)] rounded-md duration-500 ease-in-out transform ${
          mounted ? (ready ? "translate-x-0 opacity-100" : "delay-500 translate-x-0 opacity-100") : "-translate-x-full opacity-0"
        }`}
      >
        <div className="flex gap-4 pr-4 mb-2 font-semibold relative">
          ...
          {/* Main translation */}
          <p
            className={`text-xl max-w-3/4 min-h-8 flex items-center duration-200 ${
              fading.includes(mainTranslation) ? "opacity-0" : "opacity-100"
            }`}
          >
        ...
        <ul className="pl-18 pr-4 pb-4 flex flex-col gap-2">
          {alternatives.map((alt, idx) =>
              <li
                ...
                className={`list-disc w-fit duration-200 ${
                  fading.includes(alt) ? "opacity-0" : "opacity-100"
