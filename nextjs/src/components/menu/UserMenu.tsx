...
function UserMenu({ showMenu, setShowMenu, submenu, pathname }: UserMenuProps) {
  ...
  return (
    <>
      {/* --------------  Blur mask when opening menu --------------  */}
      <div
        onClick={() => {
          setShowMenu(false);
          router.push(`${pathname}`)
        }}
        className={`${showMenu ? "backdrop-blur-md" : "pointer-events-none"} fixed left-0 min-h-screen w-full z-40 duration-250`}
      ></div>
      ...
      <div
        ...
        className={`
        z-40 fixed top-12 md:top-16 w-full md:w-3/4 md:left-[25%] lg:w-1/2 lg:left-[50%] h-full flex justify-center pt-8 md:pt-10 px-4 md:pb-14
        inset-0 max-h-screen duration-400 origin-right md:border-l border-[var(--gray-1)] menu-gradient scrollbar-hidden
        ${showMenu ? "translate-x-0" : "-translate-x-300 md:translate-x-300"}`}
      >
        {(isLoading) ? (
          <div className="bg-[var(--menu)] pb-28 bg-opacity-60 z-40 flex items-center justify-center">
            <div className="spinner" />
          </div>
        ) : (
          <>
            <div
              className={`absolute w-full flex  mx-auto px-4 pt-1
            ...
            {showLogin ? (
              ...
              <div
                ...
                className={`w-full flex flex-col gap-6 items-center duration-400 ${showMenu ? "opacity-100" : "opacity-0"
                  }`}
              >
                ...
                <div className="flex flex-col gap-6 items-center w-full max-h-[calc(100dvh-8rem)] pb-8 overflow-y-scroll scrollbar-hide">
                  ...
                  <Link
                    ...
                    href="/privacy"
                    prefetch={true}
                    className={`w-full h-16 bg-[var(--bg-2)]/70 border border-[var(--gray-3)] shadow-md rounded-2xl px-6 flex items-center hover-1 shrink-0`}
                  >
                    <Shield />
