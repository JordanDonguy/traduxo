...
describe("addToFavorite", () => {
  ...
  // ------ Test 4️⃣ ------
  it("returns early if no token is provided", async () => {
    ...
    expect(setIsFavorite).toHaveBeenCalledWith(true); // called before token check
...
describe("deleteFromFavorite", () => {
  ...
  it("returns early if no token is provided", async () => {
    ...
    const result = await deleteFromFavorite(
      "abc123",
      setTranslationId,
      setIsFavorite,
      undefined, // no token
